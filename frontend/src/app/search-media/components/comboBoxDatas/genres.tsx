...
const Genres = ({ searchParams, setSearchParams }: SearchStateProps) => {
  const [genres, setGenres] = useState<Values[]>([]);
  ...
  const getData = async () => {
    ...
    const request = await fetch(`${INVENTORY_API}genres`);
    ...
    // Set the genres state with the mapped data
    setGenres(data);
    setLoading(false);
  };
  ...
  return (
    <Combobox
      ...
      notFound="Could not find any genres"
